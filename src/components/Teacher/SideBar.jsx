...
const SideBar = () => {
    const {isEnducator} = useContext(AppContext)
    const menuItems = [
        ...
        <div className='w-16 md:w-64 border-r min-h-screen text-base border-gray-500 py-2 flex flex-col'>
            {
                menuItems.map((iteam)=>(
                    ...
                    >
                        <img src={iteam.icon} alt="" className='w-6 h-6'/>
                        <p className='hidden md:block text-center'>{iteam.name}</p>
                    </NavLink>
                ))
            }
        </div>
